...
```{r ci_with_laber}
# Hmisc::rcorr provides a p-value matrix, but we don't want to introduce the dependency
# HM <- Hmisc::rcorr(as.matrix(mtcars))

# therefore, we use fake p-values
HM <- list( r = cor(mtcars),
            P = 1 - abs(cor(mtcars)) ^ .08)

corrplot(HM$r, p.mat = HM$P, insig = "label_sig",
         sig.level = c(.001, .01, .05), pch.cex = .9, pch.col = "white")
corrplot(HM$r, p.mat = HM$P, method = "color",
         insig = "label_sig", pch.col = "white")
corrplot(HM$r, p.mat = HM$P, method = "color", type = "upper",
         sig.level = c(.001, .01, .05), pch.cex = .9,
         insig = "label_sig", pch.col = "white")
corrplot(HM$r, p.mat = HM$P, insig = "label_sig", pch.col = "white",
...
colnames(M) <- paste0("f", 1:nrow(M))

# we make 30% of all features to be correlated with feature #1
idx_correlated_features <- as.integer(seq(from = 1, to = size, length.out = size * .3)[-1])
for (i in idx_correlated_features) {
  M[,i] <- M[,1] + runif(size) # adding some noise
}

corrplot(cor(M), diag = FALSE, order = "FPC",
