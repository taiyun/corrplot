  ...
  ...)
{
  ...
  bottomy <- length(correlation_matrix[,1]) -
    which(colnames(correlation_matrix) == rend) + .5
  topy <- length(correlation_matrix[,1]) -
    which(colnames(correlation_matrix) == rstart) + 1 + .5

  #uses rect function to construct the rectangle using above values as the parameters
  graphics::rect(xleft = leftx, ybottom = bottomy,
                 xright = rightx, ytop = topy,
                 lwd = lwd, ...)
  ...
  if (mirror == TRUE){
    if (is.null(cstart) == FALSE && is.null(cend) == FALSE){
      make_rect(rstart = cstart, rend = cend, cstart = rstart, cend = rend,
                correlation_matrix = correlation_matrix, mirror=FALSE, ...)
    }
  }
}
...
make_all_rects <- function(
  ...
  if(ondiag == TRUE){
    ...
    for (i in 1: (length(cutpoints)))
    {
      ...
      {
        make_rect(rstart = cutpoints[i],
                  rend = colnames(correlation_matrix)[(which(
                    colnames(correlation_matrix) == cutpoints[i + 1]) - 1)],
                  correlation_matrix = correlation_matrix,
                  lwd = ondiag_width, border = col_ondiag, ...)
      }
      #if about to make the last rectangle, use the endpoint
      else{
        make_rect(rstart = cutpoints[i], rend = endpoint,
                  correlation_matrix = correlation_matrix,
                  lwd = ondiag_width, border = col_ondiag, ...)
      }
    }
      ...
      for (s in i:(length(cutpoints) - 1)){
        ...
        if (s <= (length(cutpoints) - 2))
        {
          make_rect(rstart = cutpoints[i],
                    rend = colnames(correlation_matrix)[(which(
                      colnames(correlation_matrix) == cutpoints[i + 1]) - 1)],
                    cstart = cutpoints[s+1],
                    cend = colnames(correlation_matrix)[(which(
                      colnames(correlation_matrix) == cutpoints[s + 2]) - 1)],
                    correlation_matrix = correlation_matrix,
                    border = col_offdiag, lwd = offdiag_width,
                    mirror = TRUE, ...)
        }
          ...
          make_rect(rstart = cutpoints[i],
                    rend = colnames(correlation_matrix)[(which(
                      colnames(correlation_matrix) == cutpoints[i + 1]) - 1)],
                    cstart = cutpoints[s + 1], cend = endpoint,
                    correlation_matrix = correlation_matrix,
                    mirror = TRUE, border = col_offdiag,
                    lwd = offdiag_width, ...)
        }
      }
    }
